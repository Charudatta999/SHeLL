...
TEST(PipeTest, ConstructorCreatesValidPipes)
{
    ...
    EXPECT_GT(pipe0.GetReadPipeFD()->GetFD(), 2);
    EXPECT_GT(pipe0.GetWritePipeFD()->GetFD(), 2);

    // Read and write FDs must be distinct
    EXPECT_NE(pipe0.GetReadPipeFD()->GetFD(), pipe0.GetWritePipeFD()->GetFD());
}
...
{
    Pipe pipe0;
    int originalWriteFd = pipe0.GetWritePipeFD()->GetFD();

    // Close the read end
    pipe0.CloseReadFD();
    EXPECT_EQ(pipe0.GetReadPipeFD()->GetFD(), -1);
    ...
    EXPECT_EQ(pipe0.GetWritePipeFD()->GetFD(), originalWriteFd);

    // Close write end
    pipe0.CloseWriteFD();
    EXPECT_EQ(pipe0.GetWritePipeFD()->GetFD(), -1);
}
...
{
    Pipe pipe1;
    int readFd  = pipe1.GetReadPipeFD()->GetFD();
    int writeFd = pipe1.GetWritePipeFD()->GetFD();

    // Trigger the move constructor
    Pipe pipe2(std::move(pipe1));

    // pipe1 should now be completely empty
    EXPECT_EQ(pipe1.GetReadPipeFD()->GetFD(),  -1);
    EXPECT_EQ(pipe1.GetWritePipeFD()->GetFD(), -1);

    // pipe2 should own the original FDs
    EXPECT_EQ(pipe2.GetReadPipeFD()->GetFD(),  readFd);
    EXPECT_EQ(pipe2.GetWritePipeFD()->GetFD(), writeFd);
}
...
{
    Pipe pipe1;
    int p1Read  = pipe1.GetReadPipeFD()->GetFD();
    int p1Write = pipe1.GetWritePipeFD()->GetFD();
    ...
    EXPECT_EQ(pipe2.GetReadPipeFD()->GetFD(),  p1Read);
    EXPECT_EQ(pipe2.GetWritePipeFD()->GetFD(), p1Write);

    // Note: pipe2's original FDs were safely closed during the assignment
}

// End-to-end integration test: does it actually act like a pipe?
TEST(PipeTest, DataTransmission)
{
    ...
    ssize_t bytesWritten = write(pipe0.GetWritePipeFD()->GetFD(),
                                 testMsg.c_str(), testMsg.size());
    EXPECT_EQ(bytesWritten, static_cast<ssize_t>(testMsg.size()));

    // 2. Read data from the read end
    std::array<char, 128> buffer = {0};
    ssize_t bytesRead = read(pipe0.GetReadPipeFD()->GetFD(),
                             buffer.data(), sizeof(buffer) - 1);

    // 3. Verify it traversed the pipe perfectly
    EXPECT_EQ(bytesRead, static_cast<ssize_t>(testMsg.size()));
